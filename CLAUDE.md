...
- Mass search (1.1): `db.search_mass(delta, *, tolerance=0.01, tolerance_unit="da", site=None, position=None)` over `monoisotopic_mass`, returns `(entry, delta - mass)` closest first; `db.get_by_site(site)`. The index and site/position rules live in `_mass.py`, identical in psimodpy, unimodpy and uniprotptmpy: keep the three copies in sync.
- `entry.get_mass(*, monoisotopic=True)` (1.1) returns `monoisotopic_mass`/`average_mass` only, never a linked mass (review decision: PTM-0133 links to a 0.0 PSI-MOD mass, some links are wrong); `search_mass` indexes `get_mass()`, the same with or without the extra.
