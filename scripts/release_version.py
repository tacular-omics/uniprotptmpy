...
``sync --set X.Y.Z`` also sets CITATION.cff ``date-released`` to today (adding the
field if missing) and turns the changelog's ``## [Unreleased]`` section into
...
def set_date_released(citation: str, today: str) -> str:
    """Set CITATION.cff ``date-released`` to ``today``, adding it after ``version:`` if missing."""
    if re.search(r"^date-released:", citation, flags=re.MULTILINE):
        return replace_field(citation, r"^(date-released: *)[^\n]+$", today)
    field = f"date-released: {json.dumps(today)}\n"
    updated, count = re.subn(r"^(version: *[^\n]+\n)", lambda match: match[1] + field, citation, flags=re.MULTILINE)
    if count != 1:
        raise ValueError("Expected exactly one version field in CITATION.cff")
    return updated
